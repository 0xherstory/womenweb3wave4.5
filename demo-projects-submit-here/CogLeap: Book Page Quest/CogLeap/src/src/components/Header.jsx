...
const Header = ({ isAuthenticated, handleLogin, handleLogout, walletAddress }) => {
  
  const getWalletStatus = () => {
    if (isAuthenticated && walletAddress) {
      return `已登录: ${walletAddress.slice(0, 6)}...${walletAddress.slice(-4)}`;
    }
    return "登录 / 注册";
  };
    ...
    <header className="flex justify-between items-center mb-10 relative">
      <div className="flex items-center">
        <div className="text-neon-blue text-shadow-neon text-3xl font-bold tracking-wider">
          认知跃迁<span class="text-neon-pink text-shadow-neon-pink">书页闯关录</span>
        </div>
      ...
      <div className="flex items-center space-x-6">
        ...
        <button id="toolButton" className="bg-dark-700 hover:bg-dark-800 px-4 py-2 rounded border border-neon-blue/30 transition-all duration-300 button-glow-blue hover:button-glow-blue/50">
          <i className="fa fa-wrench mr-2 text-neon-blue"></i>思维工具库
        </button>
        <button id="rankButton" className="bg-dark-700 hover:bg-dark-800 px-4 py-2 rounded border border-neon-pink/30 transition-all duration-300 button-glow hover:button-glow/50">
          <i className="fa fa-trophy mr-2 text-neon-pink"></i>知识段位
        </button>
        ...
        <div className="relative">
          <div id="profilePic" className="w-10 h-10 rounded-full bg-dark-700 border-2 border-neon-purple flex items-center justify-center overflow-hidden">
            <img src="https://picsum.photos/200/200?random=1" alt="玩家头像" className="w-full h-full object-cover"></img>
          </div>
          {isAuthenticated && (
