...
const BookCard = ({ book, onBookSelect }) => {
  const colorClasses = {
    'neon-blue': 'border border-neon-blue/30 text-neon-blue shadow-glow-neon-blue-soft hover:shadow-glow-neon-blue',
    'neon-pink': 'border border-neon-pink/30 text-neon-pink shadow-glow-neon-pink-soft hover:shadow-glow-neon-pink',
    'neon-purple': 'border border-neon-purple/30 text-neon-purple shadow-glow-neon-purple-soft hover:shadow-glow-neon-purple',
    'neon-green': 'border border-neon-green/30 text-neon-green shadow-glow-neon-green-soft hover:shadow-glow-neon-green',
    'gray': 'border border-gray-700 cursor-not-allowed'
  };
  const iconBgClasses = {
    'neon-blue': 'border-neon-blue/40',
  ...
  const getStarRating = (count) => {
    ...
    for (let i = 0; i < 3; i++) {
        stars.push(
            <i key={i} className={`fa ${i < count ? 'fa-star text-yellow-400' : 'fa-star-o text-gray-600'}`}></i>
        );
    }
    return stars;
  };
  ...
  return (
    <div className={`bg-dark-800/70 backdrop-blur-md rounded-lg p-4 transition-all duration-300 relative overflow-hidden group ${colorClasses[book.color]} ${!book.locked ? 'cursor-pointer' : ''}`}
      onClick={!book.locked ? () => onBookSelect(book) : null}>
      {book.locked && (
...
const CampaignContent = ({ onBookSelect }) => {
  ...
  return (
    <div id="campaignMode" className="mb-8">
      <div className="flex justify-between items-center mb-4">
        ...
        <button className="text-neon-pink hover:text-neon-pink/80 transition-colors">
