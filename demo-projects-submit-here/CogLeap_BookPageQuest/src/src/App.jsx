...
const App = () => {
  ...
  const [isDashboardModalOpen, setIsDashboardModalOpen] = useState(false); // 重命名状态
  const [isThinkingToolsModalOpen, setIsThinkingToolsModalOpen] = useState(false); // 新增：工具库弹窗状态
  ...
  const handleOpenDashboardModal = () => { // 重命名函数
    if (isAuthenticated) {
      setIsDashboardModalOpen(true);
    }
  };

  const handleCloseDashboardModal = () => setIsDashboardModalOpen(false); // 重命名函数
    ...
    <>
      ...
      <div className="relative container mx-auto px-4 py-8 max-w-7xl">
        <Header 
          ...
          handleOpenDashboardModal={handleOpenDashboardModal} // 传递新函数
        ...
        <main className="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            <GameModes 
              ...
              isAuthenticated={isAuthenticated}
            />
            <Market isAuthenticated={isAuthenticated} />
          </div>
        </main>
      </div>
      ...
      {isDashboardModalOpen && (
        <DashboardModal onClose={handleCloseDashboardModal} /> // 渲染新组件
      )}

      {isThinkingToolsModalOpen && (
        <ThinkingToolsModal onClose={handleCloseThinkingToolsModal} />
      )}
    </>
  );
};
